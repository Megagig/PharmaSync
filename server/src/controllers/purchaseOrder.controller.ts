...
export const updatePurchaseOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (
    ...
    throw new AppError(`Cannot update a purchase order with status: ${purchaseOrder.status}`, 400);
  }
  ...
  if (discount !== undefined) {
    ...
    purchaseOrder.total = purchaseOrder.subtotal - discount + purchaseOrder.tax + purchaseOrder.shippingCost;
  }
  if (tax !== undefined) {
    purchaseOrder.tax = tax;
    purchaseOrder.total = purchaseOrder.subtotal - purchaseOrder.discount + tax + purchaseOrder.shippingCost;
  }
  if (shippingCost !== undefined) {
    purchaseOrder.shippingCost = shippingCost;
    purchaseOrder.total = purchaseOrder.subtotal - purchaseOrder.discount + purchaseOrder.tax + shippingCost;
  }
 ...
 * @access  Private
 */
export const addPurchaseOrderItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (
    ...
    throw new AppError(`Cannot add items to a purchase order with status: ${purchaseOrder.status}`, 400);
  }
  ...
  purchaseOrder.total = purchaseOrder.subtotal - purchaseOrder.discount + purchaseOrder.tax + purchaseOrder.shippingCost;
 ...
 * @access  Private
 */
export const updatePurchaseOrderItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (
    ...
    throw new AppError(`Cannot update items in a purchase order with status: ${purchaseOrder.status}`, 400);
  }
  ...
  // Update purchase order subtotal and total
  purchaseOrder.subtotal = purchaseOrder.subtotal - oldSubtotal + item.subtotal;
  purchaseOrder.total = purchaseOrder.subtotal - purchaseOrder.discount + purchaseOrder.tax + purchaseOrder.shippingCost;
 ...
 * @access  Private
 */
export const removePurchaseOrderItem = asyncHandler(async (req: Request, res: Response) => {
  const { id, itemId } = req.params;
  ...
  if (
    ...
    throw new AppError(`Cannot remove items from a purchase order with status: ${purchaseOrder.status}`, 400);
  }
  ...
  // Remove the item
  purchaseOrder.items.id(itemId)?.remove();
...
export const approvePurchaseOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (purchaseOrder.status !== PurchaseOrderStatus.PENDING) {
    throw new AppError(`Cannot approve a purchase order with status: ${purchaseOrder.status}`, 400);
  }
...
export const markAsOrdered = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (purchaseOrder.status !== PurchaseOrderStatus.APPROVED) {
    throw new AppError(`Cannot mark as ordered a purchase order with status: ${purchaseOrder.status}`, 400);
  }
...
export const receivePurchaseOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (
    ...
    throw new AppError(`Cannot receive a purchase order with status: ${purchaseOrder.status}`, 400);
  }
  ...
  for (const receivedItem of items) {
    const { itemId, receivedQuantity, batchNumber, expiryDate } = receivedItem;
    
    // Find the purchase order item
    const purchaseOrderItem = purchaseOrder.items.id(itemId);
    
    if (!purchaseOrderItem) {
      throw new AppError(`Purchase order item with ID ${itemId} not found`, 404);
    }
    
    // Update received quantity
    const newReceivedQuantity = (purchaseOrderItem.receivedQuantity || 0) + receivedQuantity;
    
    if (newReceivedQuantity > purchaseOrderItem.quantity) {
      throw new AppError(`Received quantity exceeds ordered quantity for item ${itemId}`, 400);
    }
    ...
    const medication = await Medication.findById(purchaseOrderItem.medication);
    ...
    medication.totalStock += receivedQuantity;
    
    await medication.save();
  }
  
  // Update purchase order
  purchaseOrder.deliveryDate = deliveryDate ? new Date(deliveryDate) : new Date();
  purchaseOrder.status = allItemsReceived ? PurchaseOrderStatus.RECEIVED : PurchaseOrderStatus.PARTIAL;
...
export const cancelPurchaseOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (
    ...
    throw new AppError(`Cannot cancel a purchase order with status: ${purchaseOrder.status}`, 400);
  }
