...
export const getAllActivityLogs = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (req.query.type || req.query.activityType) {
      filter.type = req.query.type || req.query.activityType;
    }
 ...
 * @access  Private
 */
export const getActivityTypes = asyncHandler(
  ...
  async (req: Request, res: Response) => {
    ...
    const activityByRole = await ActivityLog.aggregate([
      ...
      },
      {
        $group: {
          _id: {
            roleType: { $ifNull: ['$roleDetails.type', '$userDetails.role'] },
            roleName: { $ifNull: ['$roleDetails.name', '$userDetails.role'] },
          },
          count: { $sum: 1 },
        },
      },
