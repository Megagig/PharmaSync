...
export const createPrescription = asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const item of items) {
    ...
    if (!medication) {
      throw new AppError(`Medication with ID ${item.medication} not found`, 404);
    }
 ...
 * @access  Private
 */
export const addPrescriptionItem = asyncHandler(async (req: Request, res: Response) => {
  const { medication, dosage, quantity, refills, startDate, endDate, notes } = req.body;
  ...
  if (!medicationDoc.requiresPrescription) {
    throw new AppError(`Medication ${medicationDoc.name} does not require a prescription`, 400);
  }
 ...
 * @access  Private
 */
export const updatePrescriptionItem = asyncHandler(async (req: Request, res: Response) => {
  const { dosage, quantity, refills, refillsRemaining, endDate, notes } = req.body;
  ...
  if (refillsRemaining !== undefined) item.refillsRemaining = refillsRemaining;
 ...
 * @access  Private
 */
export const removePrescriptionItem = asyncHandler(async (req: Request, res: Response) => {
  const { id, itemId } = req.params;
  ...
  // Remove the item
  prescription.items.id(itemId)?.remove();
 ...
 * @access  Private
 */
export const dispenseMedication = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (prescription.status !== PrescriptionStatus.ACTIVE) {
    throw new AppError(`Cannot dispense medication for a prescription with status: ${prescription.status}`, 400);
  }
  ...
  for (const dispensedItem of items) {
    ...
    const prescriptionItem = prescription.items.id(itemId);
    
    if (!prescriptionItem) {
      throw new AppError(`Prescription item with ID ${itemId} not found`, 404);
    }
    ...
    const inventoryItem = medication.inventory.find(item => item.batchNumber === batchNumber);
  ...
  prescription.dispensingHistory.push({
    ...
    batchNumber: items.map(item => item.batchNumber).join(', '),
    dispensedBy: req.user.id, // From auth middleware
    notes,
  });
  
  // Update prescription status if all items have been dispensed
  const allDispensed = prescription.items.every(item => item.refillsRemaining === 0);
