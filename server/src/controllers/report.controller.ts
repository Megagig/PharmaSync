 ...
 * @access  Private
 */
export const getReportConfigurationById = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!configuration.isPublic && configuration.createdBy.toString() !== req.user.id) {
    throw new AppError('You do not have permission to access this report configuration', 403);
  }
 ...
 * @access  Private
 */
export const createReportConfiguration = asyncHandler(async (req: Request, res: Response) => {
  const { name, description, type, isPublic, filters, charts, startDate, endDate } = req.body;
 ...
 * @access  Private
 */
export const updateReportConfiguration = asyncHandler(async (req: Request, res: Response) => {
  const { name, description, isPublic, filters, charts, startDate, endDate } = req.body;
  ...
  if (configuration.createdBy.toString() !== req.user.id) {
    throw new AppError('You do not have permission to update this report configuration', 403);
  }
 ...
 * @access  Private
 */
export const deleteReportConfiguration = asyncHandler(async (req: Request, res: Response) => {
  const configuration = await ReportConfiguration.findById(req.params.id);
  ...
  if (configuration.createdBy.toString() !== req.user.id) {
    throw new AppError('You do not have permission to delete this report configuration', 403);
  }
...
export const generateReport = asyncHandler(async (req: Request, res: Response) => {
  const { type, startDate, endDate, filters, format = ReportFormat.JSON, charts } = req.body as IReportRequest;
  ...
  switch (type) {
    case ReportType.SALES:
      const salesData = await generateSalesReport(parsedStartDate, parsedEndDate, filters);
      reportData = {
        title: 'Sales Report',
        ...
        charts: [
          {
            ...
            type: 'pie',
            data: [
              { label: 'In Stock', value: inventoryData.totalItems - inventoryData.lowStockCount },
    ...
    case ReportType.PRESCRIPTION:
      const prescriptionData = await generatePrescriptionReport(parsedStartDate, parsedEndDate, filters);
      reportData = {
    ...
    case ReportType.CUSTOM:
      if (!charts || charts.length === 0) {
        throw new AppError('Custom reports require at least one chart configuration', 400);
      }
      
      // Generate data for each chart
      const chartResults = await Promise.all(
        charts.map(async (chart) => {
          const data = await generateChartData(chart, parsedStartDate, parsedEndDate, {});
          return {
            title: chart.title,
            type: chart.type,
            data,
  ...
  if (format !== ReportFormat.JSON) {
    try {
      ...
      throw new AppError(`Failed to export report: ${error.message}`, 500);
    }
  }
 ...
 * @access  Private
 */
export const generateReportFromConfiguration = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Generate the report
  try {
    const result = await generateReport(req, res);
 ...
 * @access  Private
 */
export const getReportExecutions = asyncHandler(async (req: Request, res: Response) => {
  const page = parseInt(req.query.page as string) || 1;
  const limit = parseInt(req.query.limit as string) || 10;
  const skip = (page - 1) * limit;
...
export const downloadReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (execution.executedBy.toString() !== req.user.id) {
    throw new AppError('You do not have permission to download this report', 403);
  }
 ...
 * @access  Private
 */
export const createReportSchedule = asyncHandler(async (req: Request, res: Response) => {
  const { report, frequency, format, recipients, nextRunDate, isActive } = req.body;
  ...
  if (configuration.createdBy.toString() !== req.user.id) {
    throw new AppError('You do not have permission to schedule this report', 403);
  }
 ...
 * @access  Private
 */
export const getReportSchedules = asyncHandler(async (req: Request, res: Response) => {
  const page = parseInt(req.query.page as string) || 1;
  const limit = parseInt(req.query.limit as string) || 10;
  const skip = (page - 1) * limit;
 ...
 * @access  Private
 */
export const updateReportSchedule = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (schedule.createdBy.toString() !== req.user.id) {
    throw new AppError('You do not have permission to update this schedule', 403);
  }
 ...
 * @access  Private
 */
export const deleteReportSchedule = asyncHandler(async (req: Request, res: Response) => {
  const schedule = await ReportSchedule.findById(req.params.id);
  ...
  if (schedule.createdBy.toString() !== req.user.id) {
    throw new AppError('You do not have permission to delete this schedule', 403);
  }
