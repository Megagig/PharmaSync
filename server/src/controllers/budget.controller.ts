...
export const createBudget = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const totalBudget = items.reduce((sum: number, item: any) => sum + item.amount, 0);
...
export const updateBudget = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (req.body.items) {
      budget.totalBudget = req.body.items.reduce((sum: number, item: any) => sum + item.amount, 0);
 ...
 * @access  Private (Admin/Manager only)
 */
export const closeBudget = asyncHandler(
  async (req: Request, res: Response) => {
    const budget = await Budget.findById(req.params.id);
...
export const updateBudgetActuals = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const expensesByCategory = expenses.reduce((acc: any, expense) => {
      const key = `${expense.category}${expense.subcategory ? `-${expense.subcategory}` : ''}`;
    ...
    budget.items.forEach((item) => {
      const key = `${item.category}${item.subcategory ? `-${item.subcategory}` : ''}`;
    ...
    // Calculate totals
    const totalActual = actuals.reduce((sum, actual: any) => sum + actual.amount, 0);
