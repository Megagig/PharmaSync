 ...
 * @access  Private
 */
export const getAllDispensing = asyncHandler(
 ...
 * @access  Private
 */
export const getDispensingById = asyncHandler(
  async (req: Request, res: Response) => {
 ...
 * @access  Private
 */
export const createDispensing = asyncHandler(
  async (req: Request, res: Response) => {
    const {
      patient,
      prescription,
    ...
    for (const item of items) {
      ...
      const inventoryItem = medicationDoc.inventory.find(
      ...
      const totalStock = medicationDoc.inventory.reduce(
 ...
 * @access  Private
 */
export const updateDispensing = asyncHandler(
  async (req: Request, res: Response) => {
    const { status, paymentMethod, discount, tax, notes, receiptGenerated } =
      req.body;
    ...
    if (receiptGenerated !== undefined)
      dispensing.receiptGenerated = receiptGenerated;
...
export const generateReceipt = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const receiptData = {
      ...
      patient: {
        name: (() => {
          const patient = dispensing.patient as any;
          return patient && typeof patient === 'object' && patient.firstName
            ? `${patient.firstName} ${patient.lastName}`
            : 'Unknown Patient';
        })(),
        id: (() => {
          const patient = dispensing.patient as any;
          return patient && typeof patient === 'object' && patient._id
            ? patient._id
            : 'Unknown ID';
        })(),
      },
      dispensedBy: (() => {
        const dispensedBy = dispensing.dispensedBy as any;
        return dispensedBy &&
          typeof dispensedBy === 'object' &&
          dispensedBy.firstName
          ? `${dispensedBy.firstName} ${dispensedBy.lastName}`
          : 'Unknown Staff';
      })(),
      items: dispensing.items.map((item: any) => ({
        medication:
          typeof item.medication === 'object' && item.medication.name
 ...
 * @access  Private
 */
export const returnDispensing = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (dispensing.status !== DispensingStatus.COMPLETED) {
      throw new AppError(
        `Cannot return a dispensing with status: ${dispensing.status}`,
        400
      );
    }
    ...
    for (const returnedItem of items) {
      ...
      const dispensingItem = dispensing.items.find(
      ...
      if (quantity > dispensingItem.quantity) {
        throw new AppError(
          `Return quantity exceeds dispensed quantity for item ${itemId}`,
          400
        );
      }
      ...
      const inventoryItem = medication.inventory.find(
        (item) => item.batchNumber === dispensingItem.batchNumber
      );
