...
export const getLowStockAlerts = asyncHandler(async (req: Request, res: Response) => {
  ...
  const lowStockMedications = await Medication.find({
    ...
    .select('name genericName brandName strength dosageForm totalStock reorderLevel')
    .sort({ totalStock: 1 });
 ...
 * @access  Private
 */
export const getExpiringStockAlerts = asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const medication of medications) {
    const expiringBatches = medication.inventory.filter(
      (batch) => batch.expiryDate <= expiryDate && batch.expiryDate >= new Date() && batch.quantity > 0
    );
...
export const getInventoryValuation = asyncHandler(async (req: Request, res: Response) => {
  ...
  const medications = await Medication.find().select('name inventory totalStock');
 ...
 * @access  Private
 */
export const getInventoryMovement = asyncHandler(async (req: Request, res: Response) => {
...
export const adjustInventory = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (batchIndex === -1) {
    throw new AppError(`Batch ${batchNumber} not found for this medication`, 404);
  }
  ...
  // Add adjustment note
  const adjustmentNote = `Inventory adjusted by ${quantity > 0 ? '+' : ''}${quantity} units. Reason: ${reason}`;
