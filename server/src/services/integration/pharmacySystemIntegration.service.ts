...
class PharmacySystemIntegrationService {
  ...
  constructor() {
    ...
    this.baseUrl = config.integrations.pharmacySystem.baseUrl || 'https://api.pharmacysystem.example.com';
  ...
  async getInventory(): Promise<any[]> {
    ...
    try {
      ...
      logger.error(`Error fetching inventory from pharmacy system: ${error.message}`);
      throw new AppError(`Failed to fetch inventory from pharmacy system: ${error.message}`, 500);
    }
  }
  ...
  async syncInventory(): Promise<any[]> {
    ...
    try {
      ...
      logger.error(`Error syncing inventory with pharmacy system: ${error.message}`);
      throw new AppError(`Failed to sync inventory with pharmacy system: ${error.message}`, 500);
    }
  }
  ...
  async sendPrescription(prescriptionData: any): Promise<any> {
    ...
    try {
      ...
      // Send prescription to pharmacy system
      const response = await axios.post(`${this.baseUrl}/prescriptions`, mappedData, {
        headers: {
          'Authorization': `Bearer ${this.apiKey}`,
          'Content-Type': 'application/json',
        },
      ...
      logger.error(`Error sending prescription to pharmacy system: ${error.message}`);
      throw new AppError(`Failed to send prescription to pharmacy system: ${error.message}`, 500);
    }
  }
  ...
  async getDispensingRecords(startDate?: Date, endDate?: Date): Promise<any[]> {
    ...
    try {
      ...
      logger.error(`Error fetching dispensing records from pharmacy system: ${error.message}`);
      throw new AppError(`Failed to fetch dispensing records from pharmacy system: ${error.message}`, 500);
    }
  }
  ...
  private mapPrescriptionToExternalFormat(prescriptionData: any): any {
    return {
      ...
      patientId: prescriptionData.patient?.ehrPatientId || prescriptionData.patient?._id.toString(),
      ...
      medications: prescriptionData.medications.map((med: any) => ({
        medicationId: med.medication?.externalId || med.medication?._id.toString(),
