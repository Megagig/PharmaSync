 ...
 */
export const generatePDF = (data: any, title: string): Buffer => {
    ...
    try {
      ...
      Object.entries(data).forEach(([key, value]) => {
        if (key !== 'summary' && Array.isArray(value) && value.length > 0) {
          ...
          if (value.length <= 10) { // Limit to 10 items for readability
            value.forEach((item, index) => {
              const itemText = Object.entries(item)
                .map(([itemKey, itemValue]) => {
                  const formattedKey = itemKey.replace(/([A-Z])/g, ' $1')
                    .replace(/^./, (str) => str.toUpperCase());
                  
                  let formattedValue = itemValue;
                  // Format currency values
                  if (itemKey.toLowerCase().includes('price') || 
            ...
            doc.fontSize(10).text(`[${value.length} items - too many to display in PDF format]`);
          }
  ...
  Object.entries(data).forEach(([key, value]) => {
    if (key !== 'summary' && Array.isArray(value) && value.length > 0) {
      ...
      if (value.length > 0) {
        ...
        value.forEach((item) => {
          headers.forEach((header, colIndex) => {
            sheet.getCell(rowIndex, colIndex + 1).value = item[header];
          });
          rowIndex += 1;
        });
...
export const generateCSV = (data: any): Buffer => {
  try {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      if (key === 'summary' && typeof value === 'object' && !Array.isArray(value)) {
        // Handle summary object
        Object.entries(value).forEach(([summaryKey, summaryValue]) => {
          if (typeof summaryValue !== 'object') {
            const formattedKey = summaryKey.replace(/([A-Z])/g, ' $1')
              .replace(/^./, (str) => str.toUpperCase());
