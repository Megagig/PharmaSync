...
const config = {
  ...
  integrations: {
    ...
    ehr: {
      ...
      baseUrl:
        process.env.EHR_API_BASE_URL || 'https://api.ehrsystem.example.com',
    ...
    pharmacySystem: {
      ...
      baseUrl:
        process.env.PHARMACY_SYSTEM_API_BASE_URL ||
        'https://api.pharmacysystem.example.com',
      apiKey: process.env.PHARMACY_SYSTEM_API_KEY || '',
    },

    // Drug Information Database (RxNav)
    drugDatabase: {
      enabled: process.env.DRUG_DATABASE_INTEGRATION_ENABLED || 'true',
      baseUrl:
        process.env.DRUG_DATABASE_API_BASE_URL ||
        'https://rxnav.nlm.nih.gov/REST',
      apiKey: process.env.DRUG_DATABASE_API_KEY || '', // RxNav doesn't require an API key
    },
  },
  ...
  fileUpload: {
    ...
    allowedTypes: (
      process.env.FILE_UPLOAD_ALLOWED_TYPES ||
