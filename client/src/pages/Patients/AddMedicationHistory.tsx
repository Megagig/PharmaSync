...
import { fetchPatientById, addMedicationHistory, setError } from '@/store/slices/patientSlice';
    ...
    if (id) {
      try {
        const resultAction = await dispatch(
          addMedicationHistory({ patientId: id, medicationData: data })
        );
        if (addMedicationHistory.fulfilled.match(resultAction)) {
          navigate(`/patients/${id}`);
        }
      } catch (error) {
        console.error('Failed to add medication history:', error);
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-gray-900">Add Medication History</h1>
      </div>
      ...
      <Card>
        <MedicationHistoryForm onSubmit={handleSubmit} isLoading={isLoading} />
      </Card>
    </div>
  );
};
