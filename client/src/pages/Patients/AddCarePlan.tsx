...
import { fetchPatientById, addCarePlan, setError } from '@/store/slices/patientSlice';
    ...
    if (id) {
      try {
        const resultAction = await dispatch(
          addCarePlan({ patientId: id, planData: data })
        );
        if (addCarePlan.fulfilled.match(resultAction)) {
          navigate(`/patients/${id}`);
        }
      } catch (error) {
        console.error('Failed to add care plan:', error);
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-gray-900">Add Care Plan</h1>
      </div>
      ...
      <Card>
        <CarePlanForm onSubmit={handleSubmit} isLoading={isLoading} />
      </Card>
    </div>
  );
};
