...
import { fetchPatientById, removeDrugTherapyProblem, setError } from '@/store/slices/patientSlice';
import { DrugTherapyProblem } from '@/types/patient.types';
import Card from '@/components/common/Card/Card';
import Button from '@/components/common/Button/Button';
import { formatDate } from '@/utils/date.utils';

const DeleteDrugTherapyProblem = () => {
    ...
    if (id && problemId) {
      try {
        const resultAction = await dispatch(
          removeDrugTherapyProblem({
            patientId: id,
            problemId,
          })
        );
        if (removeDrugTherapyProblem.fulfilled.match(resultAction)) {
          navigate(`/patients/${id}`);
        }
      } catch (error) {
        console.error('Failed to delete drug therapy problem:', error);
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-gray-900">Delete Drug Therapy Problem</h1>
      </div>
      ...
      <Card>
        <div className="p-4">
          <h2 className="text-lg font-medium text-gray-900 mb-4">
            Are you sure you want to delete this drug therapy problem?
          </h2>
...
export default DeleteDrugTherapyProblem;
