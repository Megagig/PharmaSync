...
import { fetchPatientById, addSoapNote, setError } from '@/store/slices/patientSlice';
    ...
    if (id) {
      try {
        const resultAction = await dispatch(
          addSoapNote({ patientId: id, noteData: data })
        );
        if (addSoapNote.fulfilled.match(resultAction)) {
          navigate(`/patients/${id}`);
        }
      } catch (error) {
        console.error('Failed to add SOAP note:', error);
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-gray-900">Add SOAP Note</h1>
      </div>
      ...
      <Card>
        <SoapNoteForm onSubmit={handleSubmit} isLoading={isLoading} />
      </Card>
    </div>
  );
};
