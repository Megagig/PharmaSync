...
import { fetchMedicationById, addInventoryItem, setError } from '@/store/slices/medicationSlice';
import { InventoryItemFormData } from '@/types/medication.types';
import Card from '@/components/common/Card/Card';
import InventoryItemForm from '@/components/domain/Medications/InventoryItemForm';
...
const AddInventoryItem = () => {
  ...
  useEffect(() => {
    if (id && !currentMedication) {
      dispatch(fetchMedicationById(id));
    }
    
    // Clear any previous errors when component mounts
    dispatch(setError(null));
  }, [dispatch, id, currentMedication]);

  const handleSubmit = async (data: InventoryItemFormData) => {
    if (id) {
      try {
        const resultAction = await dispatch(
          addInventoryItem({ medicationId: id, inventoryData: data })
        );
        if (addInventoryItem.fulfilled.match(resultAction)) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <InventoryItemForm onSubmit={handleSubmit} isLoading={isLoading} />
      </Card>
    </div>
  );
};
