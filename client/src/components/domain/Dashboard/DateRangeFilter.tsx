...
const DateRangeFilter: React.FC<DateRangeFilterProps> = ({ onRangeChange }) => {
  ...
  const handlePresetChange = (preset: DateRangePreset) => {
    ...
    // Fetch dashboard stats with the new date range
    dispatch(fetchDashboardStats({
      startDate: startDate ? formatDateToISO(startDate) : undefined,
  ...
  const applyCustomRange = () => {
    ...
    // Fetch dashboard stats with the custom date range
    dispatch(fetchDashboardStats({
      startDate: dateRange.startDate ? formatDateToISO(dateRange.startDate) : undefined,
      endDate: dateRange.endDate ? formatDateToISO(dateRange.endDate) : undefined,
    }));
  };
          ...
          <p className="text-sm text-gray-500">
            {selectedPreset === 'all' 
              ...
              : selectedPreset === 'custom' && dateRange.startDate && dateRange.endDate
                ? `Showing statistics from ${dateRange.startDate.toLocaleDateString()} to ${dateRange.endDate.toLocaleDateString()}`
                : selectedPreset === '7days'
                  ? 'Showing statistics for the last 7 days'
                  : selectedPreset === '30days'
                    ? 'Showing statistics for the last 30 days'
                    : selectedPreset === '90days'
                      ? 'Showing statistics for the last 90 days'
                      : selectedPreset === '6months'
                        ? 'Showing statistics for the last 6 months'
                        : 'Showing statistics for the last year'}
          </p>
        </div>
